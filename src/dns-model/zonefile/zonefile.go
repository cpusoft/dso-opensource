...
import (
	...
	rr "dns-model/rr"
	gozonefile "github.com/bwesterb/go-zonefile"
...
func LoadFromZoneFile(zoneFileName string) (originModel *rr.OriginModel, err error) {
	// Load zonefile
	...
	for i, e := range zf.Entries() {
		...
		if len(e.Command()) > 0 {
			...
			if string(e.Command()) == "$ORIGIN" && len(e.Values()) > 0 {
				originModel.Origin = rr.FormatRrOrigin(string(e.Values()[0]))
			...
			rrName := rr.FormatRrName(string(e.Domain()))
			...
			if e.TTL() != nil {
				...
				if ttl > dnsutil.DSO_ADD_RECOURCE_RECORD_MAX_TTL {
					belogs.Error("LoadFromZoneFile(): ttl is bigger than DSO_ADD_RECOURCE_RECORD_MAX_TTL:", ttl, dnsutil.DSO_ADD_RECOURCE_RECORD_MAX_TTL)
			...
			rrModel := rr.NewRrModel(originModel.Origin, rrName,
...
func SaveToZoneFile(originModel *rr.OriginModel, file string) (err error) {
